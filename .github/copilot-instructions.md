...
1. Use masked placeholders (e.g. `[MASKED-EMAIL]`, `[MASKED-KEY]`, `[MASKED-ID]`) when referencing sensitive values — never reconstruct or reveal the originals.
...
Masking applies to: email addresses, private keys, and any additional patterns configured in `.github/hooks/masking-config.json`.
