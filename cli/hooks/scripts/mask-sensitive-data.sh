...
PROJECT_CONFIG_PATH="$CWD/.github/hooks/masking-config.json"
COPILOT_HOME_DIR="$(dirname "$(dirname "$SCRIPT_DIR")")"
GLOBAL_CONFIG_PATH="$COPILOT_HOME_DIR/masking-config.json"
...
MASKED_CACHE_DIR="$(printf '%s' "${TMPDIR:-/tmp}/copilot-mask-cache" | sed 's#//#/#g')"
mkdir -p "$MASKED_CACHE_DIR" 2>/dev/null || true
...
masked_temp_path() {
  ...
  ext="${source_path##*.}"
  if [ "$ext" = "$source_path" ]; then
    ext="txt"
  fi

  if command -v sha256sum >/dev/null 2>&1; then
    hash=$(printf '%s' "$source_path" | sha256sum | awk '{print $1}')
  elif command -v shasum >/dev/null 2>&1; then
    hash=$(printf '%s' "$source_path" | shasum -a 256 | awk '{print $1}')
  elif command -v openssl >/dev/null 2>&1; then
    hash=$(printf '%s' "$source_path" | openssl dgst -sha256 | awk '{print $NF}')
  else
    hash=$(printf '%s' "$source_path" | cksum | awk '{print $1}')
  fi

  printf '%s/masked-%s.%s' "$MASKED_CACHE_DIR" "$hash" "$ext"
}

updated_tool_args_with_path() {
  local original_args="$1"
  local new_path="$2"

  printf '%s' "$original_args" | jq -c --arg newPath "$new_path" '
    if type == "object" then
      if has("path") then .path = $newPath
      elif has("filePath") then .filePath = $newPath
      elif has("file_path") then .file_path = $newPath
      else . + {path: $newPath}
      end
    else
      {path: $newPath}
    end
  ' 2>/dev/null || printf '{"path":"%s"}' "$new_path"
}
...
case "$HOOK_EVENT" in
  ...
  "PreToolUse")
    ...
    if [ -n "$TOOL_INPUT_STR" ] && [ "$TOOL_INPUT_STR" != "{}" ] && [ "$TOOL_INPUT_STR" != "null" ]; then
      ...
      if [ "$TOOL_NAME" = "view" ] || [ "$TOOL_NAME" = "read_file" ] || [ "$TOOL_NAME" = "readFile" ]; then
          ...
          if [ -f "$FILE_PATH" ]; then
            ...
            if [ -n "$FILE_CONTENT" ] && has_sensitive "$FILE_CONTENT"; then
              ...
              if [ "$TOOL_NAME" = "view" ]; then
                MASKED_PATH=$(masked_temp_path "$FILE_PATH")
                printf '%s' "$MASKED_CONTENT" > "$MASKED_PATH"
                UPDATED_INPUT=$(updated_tool_args_with_path "$TOOL_INPUT_STR" "$MASKED_PATH")
                audit_log "PreToolUse" "REDIRECTED view to masked temp file for sensitive content"
                jq -n \
  ...
  "SubagentStart")
    AGENT_TYPE=$(printf '%s' "$INPUT" | jq -r '.agent_name // .agentName // .agent_display_name // "unknown"')
  ...
  "PostToolUse")
    ...
    TOOL_RESPONSE=$(printf '%s' "$INPUT" | jq -c '.tool_result // .toolResult // .tool_response // .toolResponse // .output // empty')
